...
public class ArbolBinarioBusqueda<E> {
    ...
    public List<E> getRecorridoHasta(E value) {
        List<E> list = new ArrayList<>();
        getRecorridoHasta(raiz, list, value);
        return list;
    }

    private void getRecorridoHasta(Nodo<E> rsa, List<E> list, E value) {
        if(rsa == null){
            return;
        } else {
            if(buscar(value)){
                final int comparacion = comparar(value, rsa.getDato());
                if(comparacion == 0) { //obj == rsa.getDato()
                    //Aqui encontramos el que queremos eliminar
                    list.add(rsa.getDato());
                } else if (comparacion > 0) { // si obj > rsa.getDato()
                    list.add(rsa.getDato());
                    getRecorridoHasta(rsa.getDerecho(), list, value);
                } else if (comparacion < 0) { // si obj < rsa.getDato()
                    list.add(rsa.getDato());
                    getRecorridoHasta(rsa.getIzquierdo(), list, value);
                }
            }
        }
    }
