...
public class ArbolBinarioAvl<E> {
    ...
    public ArbolBinarioAvl(Comparator<E> comparator) {
        this();
    ...
    private Nodo<E> insertar(E obj, Nodo<E> rsa) {
        if (rsa == null) {
            ...
            if (comparacion > 0) { // si obj > rsa.getDato()
                ...
                int alturaSI = getAlturaAvl(rsa.getIzquierdo());
                int alturaSD = getAlturaAvl(rsa.getDerecho());

                int factorEquilibrio = alturaSD - alturaSI;

                if (factorEquilibrio == 2) {

                    alturaSI = getAlturaAvl(rsa.getDerecho().getIzquierdo());
                    alturaSD = getAlturaAvl(rsa.getDerecho().getDerecho());

                    factorEquilibrio = alturaSD - alturaSI;
                ...
                if (factorEquilibrio == -2) {
                    ...
                    alturaSI = getAlturaAvl(rsa.getIzquierdo().getIzquierdo());
                    alturaSD = getAlturaAvl(rsa.getIzquierdo().getDerecho());
        ...
        int alturaSI = getAlturaAvl(rsa.getIzquierdo());
        int alturaSD = getAlturaAvl(rsa.getDerecho());
        int alturaRSA = Math.max(alturaSI, alturaSD) + 1;
        rsa.setAltura(alturaRSA);
        return rsa;
    }
    ...
    private int getAlturaAvl(Nodo<E> rsa) {
        if (rsa == null)
            return -1;
        else
            return rsa.getAltura();
    }

    private Nodo<E> rotacionSimpleIzquierda(Nodo<E> rsa) {

        Nodo<E> aux = rsa.getDerecho();
        rsa.setDerecho(aux.getIzquierdo());
        aux.setIzquierdo(rsa);

        return aux;
    }

    private Nodo<E> rotacionSimpleDerecha(Nodo<E> rsa) {

        Nodo<E> aux = rsa.getIzquierdo();
        rsa.setIzquierdo(aux.getDerecho());
        aux.setDerecho(rsa);

        return aux;
    }

    private Nodo<E> rotacionDobleIzquierda(Nodo<E> rsa) {

        Nodo<E> aux = rotacionSimpleDerecha(rsa.getDerecho());
        rsa.setDerecho(aux);

        aux = rotacionSimpleIzquierda(rsa);
        return aux;
    }

    private Nodo<E> rotacionDobleDerecha(Nodo<E> rsa) {

        Nodo<E> aux = rotacionSimpleIzquierda(rsa.getIzquierdo());
        rsa.setIzquierdo(aux);

        aux = rotacionSimpleDerecha(rsa);
        return aux;
